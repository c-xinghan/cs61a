...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        oprt = scheme_eval(first, env)
        validate_procedure(oprt)
        if isinstance(oprt, MacroProcedure):
            return scheme_eval(oprt.apply_macro(rest, env), env)
        return scheme_apply(oprt, rest.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        def generate_pair(formals, vals):
            if formals is not nil:
                yield formals.first, vals.first
                if formals.rest is not nil:
                    yield from generate_pair(formals.rest, vals.rest)

        pairs = {formal: val for formal, val in generate_pair(formals, vals)}

        for val in pairs:
            new_frame.define(val, pairs.get(val))
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    evaluated = scheme_eval(expressions.first, env, expressions.rest is nil)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair):
        ...
        macro_formals = target.rest
    else:
        macro_name = target
        macro_formals = nil
    validate_form(expressions.rest.first, 1)
    env.define(macro_name, MacroProcedure(macro_formals, expressions.rest, env))
    return macro_name
    # END Problem 20
