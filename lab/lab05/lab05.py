...
def couple(lst1, lst2):
    ...
    return [list(element_pair) for element_pair in zip(lst1, lst2)]
...
def closer_city(lat, lon, city1, city2):
    ...
    if distance(current_location, city1) > distance(current_location, city2):
...
def add_trees(t1, t2):
    ...
    t1b_length, t2b_length = len(branches(t1)), len(branches(t2))
    length_diff = abs(t1b_length - t2b_length)

    if t1b_length != t2b_length:
        if len(branches(t1)) < len(branches(t2)):
            t1 = tree(label(t1), branches(t1) + [tree(0) for _ in range(length_diff)])
        else:
            t2 = tree(label(t2), branches(t2) + [tree(0) for _ in range(length_diff)])

    return tree(label_sum, [add_trees(b1, b2) for b1, b2 in zip(branches(t1), branches(t2))])
...
def build_successors_table(tokens):
    ...
    for word in tokens:
        ...
        # "*** YOUR CODE HERE ***"
            ...
            table.update({prev : table.get(prev) + [word]})
        prev = word
    return table
