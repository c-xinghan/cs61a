...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def check_for_bee(place):
            ...
            if bee is not None and distance >= self.min_range:
                return bee

            # recursive call
            distance += 1
            return check_for_bee(place.entrance)

        return check_for_bee(self.place)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_impostor:
            ...
            self.throw_at(self.nearest_bee(gamestate.beehive))
            self.buff_ants(self.place.exit)
    ...
    def buff_ants(self, place, multiplier=2):
        if place is None:
            return
        if place.ant is not None:

            if hasattr(place.ant, 'contained_ant') and place.ant.contained_ant is not None and place.ant.contained_ant not in self.ants_buffed:
                place.ant.contained_ant.damage *= multiplier
                self.ants_buffed.append(place.ant.contained_ant)

            elif place.ant not in self.ants_buffed:
                place.ant.damage *= multiplier
                self.ants_buffed.append(place.ant)
                
        self.buff_ants(place.exit)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if not self.direction_forward:
            ...
            # print("DEBUG:", "knock back to True")
            # self.direction_forward = True
        # END EC
        if self.blocked():
...
def make_slow(action, bee):
    ...
    def alt_action(gamestate):
        # print("DEBUG:", "in slow", bee.direction_forward)
        if gamestate.time % 2 == 0:
            action(gamestate)
        # else:
        #     bee.direction_forward = True
...
def make_scare(action, bee):
    ...
    def alt_action(gamestate):
        # print("DEBUG:", "in scared", bee.direction_forward)
        bee.direction_forward = False
        action(gamestate)
        bee.direction_forward = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while isinstance(place, Place):
            ant_within = place.ant
            if ant_within and not ant_within is self:
                insects_and_distances[ant_within] = distance
                if hasattr(ant_within, "contained_ant") and ant_within.contained_ant:
                    insects_and_distances[ant_within.contained_ant] = distance
